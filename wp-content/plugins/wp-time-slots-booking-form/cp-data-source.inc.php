...
add_action( 'init', 'cptslotsb_init_ds', 0 );

function cptslotsb_init_ds()
	{
		if( isset( $_REQUEST[ 'cffaction' ] ) )
		{
			switch( $_REQUEST[ 'cffaction' ] )
			{
				...
				case 'test_db_query':
					...
					{
						_cptslotsb_function_check_for_variable( $_REQUEST[ 'table' ] );
						_cptslotsb_function_check_for_variable( $_REQUEST[ 'where' ] );
					}
					else
					{
						_cptslotsb_function_check_for_variable( $_REQUEST[ 'query' ] );
					}
				case 'get_data_from_database':
					...
					$query_result =  cptslotsb_ds( $_REQUEST );
					$err = mysqli_error( $cpcff_db_connect );
					if( !is_null( mysqli_connect_error() ) ) $err .= mysqli_connect_error();
				...
				case 'get_posts':
					...
					$result_obj = new stdClass;
					$result_obj->data = cptslotsb_ds( $_REQUEST );
					print( json_encode( $result_obj ) );
					exit;
				break;
	...
	} // End cpabc_appointments_init_ds

function cptslotsb_ds( $data )
	{
		switch( $data[ 'data_source' ] )
		{
			case 'database':
				return cptslotsb_ds_db( $data );
			break;
			case 'csv':
				return cptslotsb_ds_csv( $data );
			break;
			case 'post_type':
				return cptslotsb_ds_post_type( $data );
			break;
			case 'taxonomy':
				return cptslotsb_ds_taxonomy( $data );
			break;
			case 'user':
				return cptslotsb_ds_user( $data );
			break;
		}
	}
...
**/	
function _cptslotsb_function_check_for_variable( $str )
	{
...
function _cptslotsb_function_set_attr( &$obj, $attr, $arr, $elem )
	{
		$arr = (array)$arr;
...
function cptslotsb_ds_db( $data )
	{
		global $wpdb, $cpcff_db_connect, $cp_tslotsb_plugin;
		...
		{
			$connection_data = unserialize( mcrypt_decrypt( MCRYPT_RIJNDAEL_256, $cp_tslotsb_plugin->get_option( 'form_structure', CP_TSLOTSBOOK_DEFAULT_form_structure, $data[ 'form' ] ), base64_decode( $data[ 'connection' ] ), MCRYPT_MODE_ECB ) );
...
function cptslotsb_ds_csv( $data )
	{
	}
	
function cptslotsb_ds_post_type( $data )
	{
		...
		if( !empty( $data[ 'id' ] ) )
		{
			...
			{
				$tmp = new stdClass;
				_cptslotsb_function_set_attr( $tmp, 'value', $result, $data[ 'value' ] );
				_cptslotsb_function_set_attr( $tmp, 'text',  $result, $data[ 'text' ] );
...
function cptslotsb_ds_taxonomy( $data )
	{
		$taxonomies = array();
...
function cptslotsb_ds_user( $data )
	{
		$users = array();
		if( !empty( $data[ 'logged' ] ) && $data[ 'logged' ] !== 'false' )
		{
			...
			{
				$tmp = new stdClass;
				_cptslotsb_function_set_attr( $tmp, 'value', $result->data, $data[ 'value' ] );
				$users[] = $tmp;
			}
		}
		...
		else
		{
			...
			foreach( $results as $result )
			{
				$tmp = new stdClass;
				_cptslotsb_function_set_attr( $tmp, 'value', $result->data, $data[ 'value' ] );
				_cptslotsb_function_set_attr( $tmp, 'text', $result->data, $data[ 'text' ] );
				$users[] = $tmp;
			}
		}
