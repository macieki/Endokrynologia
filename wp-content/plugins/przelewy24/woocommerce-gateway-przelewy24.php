...
register_activation_hook(__FILE__, 'woocommerce_gateway_przelewy24_payment_activate');

function woocommerce_gateway_przelewy24_payment_activate()
{
    global $wpdb;
    $wpdb->query('
          CREATE TABLE IF NOT EXISTS ' . $wpdb->prefix . 'woocommerce_p24_data(
...
function woocommerce_p24_init()
{
    ...
    if ( ! check_woocommerce_version_compatibility( 4, 5 ) ) {
    ...
    add_action('woocommerce_order_action_wc_przelewy24_email_action', 'woocommerce_p24_email_send_for_order');
    add_action('woocommerce_checkout_update_order_meta', 'woocommerce_p24_email_send_for_order_id');
...
function my_stored_cards()
{
    ...
    if (isset($_POST['act']) && $_POST['act'] == 'cc_forget') {
        ...
        if (isset($_POST['cc_forget']) && $_POST['cc_forget'] == '1') {
 ...
 */
function woocommerce_p24_email_send_for_order( $order ) {
 ...
 * @param int $order_id
 */
function woocommerce_p24_email_send_for_order_id( $order_id ) {
    woocommerce_p24_email_send_for_order( new WC_Order( $order_id ) );
}
