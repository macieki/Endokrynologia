...
class P24_Config_Menu {
	...
	private function render_config_main_page() {
		if ( $this->plugin_core->is_multi_currency_active() ) {
			$this->render_config_main_page_extended();
	 ...
	 */
	private function render_config_main_page_extended() {
		$value            = $this->plugin_core->should_activate_multi_currency();
		...
		$params = compact( 'value', 'currency_options', 'report_currency' );
		$this->plugin_core->render_template( 'multi-currency-main', $params );
	}
