...
class MVVWB_Front_End
{
    ...
    function __construct( $file = '', $version = '1.0.0' )
    {
        ...
        new MVVWB_Booking_Order_Manager();
        new MVVWB_Cron_Manager();
        add_action( 'init', array( $this, 'init' ) );
        add_action( 'init', array( $this, 'cancel_booking' ) );
    ...
    {
        $gateways[] = 'MVVWB_Payment_Gateway';
        return $gateways;
    }
    ...
    public function conf_required_message( $cart_item, $cart_item_key )
    {
        ...
        if ( isset( $cart_item[MVVWB_CART_ITEM_KEY] ) ) {
            $requireConf = $cart_item[MVVWB_CART_ITEM_KEY]['requireConf'];
            if ( $requireConf ) {
                echo  '<p class="mvvwb_confirmation_tag">' . mvvwb_getConfig( 'message.confRequired', 'Confirmation Required' ) . '</p>' ;
            }
        }
    ...
    public function bookings_endpoint_content( $current_page )
    {
        ...
        $current_page = ( empty($current_page) ? 1 : absint( $current_page ) );
        $today_bookings = MVVWB_Booking::get_bookings_for_user( $user_id, array(
            'order_by'    => 'start_date',
            'order'       => 'ASC',
            'date_after'  => $now,
        ...
        $past_bookings = MVVWB_Booking::get_bookings_for_user( $user_id, array(
            'order_by'    => 'start_date',
            'order'       => 'DESC',
            'date_before' => $now,
            'offset'      => ($current_page - 1) * $bookings_per_page,
        ...
        $upcoming_bookings = MVVWB_Booking::get_bookings_for_user( $user_id, array(
            'order_by'   => 'start_date',
            'order'      => 'ASC',
        ...
        if ( !empty($today_bookings) ) {
            $tables['today'] = array(
                'header'   => __( 'Today\'s Bookings', 'bookings-for-woocommerce' ),
                'bookings' => $today_bookings,
            );
        }
        if ( !empty($upcoming_bookings) ) {
            $tables['upcoming'] = array(
                'header'   => __( 'Upcoming Bookings', 'bookings-for-woocommerce' ),
                'bookings' => $upcoming_bookings,
            );
        }
        if ( !empty($past_bookings) ) {
            $tables['past'] = array(
                'header'   => __( 'Past Bookings', 'bookings-for-woocommerce' ),
    ...
    public function cancel_booking()
    {
        ...
        if ( isset( $_GET['cancel_booking'] ) && isset( $_GET['booking_id'] ) ) {
            ...
            $booking_can_cancel = false;
            if ( in_array( $booking->get_status(), get_mvvwb_booking_statuses( 'cancel' ) ) ) {
                $booking_can_cancel = true;
            }
            $redirect = $_GET['redirect'];
            
            if ( $booking->get_status() == 'cancelled' ) {
                // Already cancelled - take no action
            } elseif ( $booking_can_cancel && $booking->getId() == $booking_id && isset( $_GET['_wpnonce'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'mvvwb-cancel_booking' ) ) {
                // Cancel the booking
                $booking->update_status( 'cancelled', true );
                do_action( 'mvvwb_cancelled_by_customer', $booking->getId() );
                // Message
                wc_add_notice( apply_filters( 'mvvwb_cancelled_notice', __( 'Your booking was cancelled.', 'booking-for-woocommerce' ) ), apply_filters( 'woocommerce_booking_cancelled_notice_type', 'notice' ) );
            } elseif ( !$booking_can_cancel ) {
                wc_add_notice( __( 'Your booking can no longer be cancelled. Please contact us if you need assistance.', 'booking-for-woocommerce' ), 'error' );
            } else {
                wc_add_notice( __( 'Invalid booking.', 'booking-for-woocommerce' ), 'error' );
            }
    ...
    {
        $booking_ids = MVVWB_Booking::get_booking_ids_from_order_item_id( $item_id );
