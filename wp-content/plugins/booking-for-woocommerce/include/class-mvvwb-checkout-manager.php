...
class MVVWB_Checkout_Manager {
	...
	public function remove_payment_methods( $available_gateways ) {
		...
		if ( mvvwb_cart_requires_confirmation() ) {
			...
			$available_gateways['mvvwb-payment-gateway'] = new MVVWB_Payment_Gateway();
		}
	...
	public function booking_requires_confirmation( $needs_payment, $cart ) {
		if ( ! $needs_payment ) {
			foreach ( $cart->cart_contents as $cart_item ) {
				if ( mvvwb_booking_requires_confirmation( $cart_item['product_id'] ) ) {
					$needs_payment = true;
					break;
				}
			}
	...
	public function booking_order_requires_confirmation( $needs_payment, $order, $valid_order_statuses ) {
		if ( ! $needs_payment && $order->has_status( $valid_order_statuses ) ) {
			...
			foreach ( $order_items as $order_item ) {
				if ( mvvwb_booking_requires_confirmation( $order_item['product_id'] ) ) {
					$needs_payment = true;
					break;
				}
			}
