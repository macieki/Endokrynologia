...
if (!function_exists('dateFormatPhpToJs')) {
    function dateFormatPhpToJs($str)
    {
        return str_replace('s', '', str_replace('S', '', $str));
    }
}
if (!function_exists('dateStringToObject')) {

    function dateStringToObject($start, $end = false)
    {
...
if (!function_exists('get_mvvwb_booking_statuses')) {
    function get_mvvwb_booking_statuses($context = 'fully_booked', $include_translation_strings = false)
    {
        if ('user' === $context) {
            $statuses = array(
                'unpaid' => __('Unpaid', ''),
